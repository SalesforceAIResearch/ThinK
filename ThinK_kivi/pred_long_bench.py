...
if __name__ == '__main__':
    ...
    if 'llama' in model_args.model_name_or_path.lower() or 'longchat' in model_args.model_name_or_path.lower():
        if model_args.k_bits < 16 and model_args.v_bits < 16:
            if model_args.pruning_ratio > 0:
                from models.llama_kivi_think import LlamaForCausalLM_KIVI
            else:
            ...
            config.pruning_ratio = model_args.pruning_ratio
    ...
    for dataset in datasets:
        if data_args.e:
            ...
            if not os.path.exists(f"pred/{model_name}_{max_length}_{model_args.k_bits}bits_group{model_args.group_size}_residual{model_args.residual_length}_ratio{model_args.pruning_ratio}_"):
                os.makedirs(f"pred/{model_name}_{max_length}_{model_args.k_bits}bits_group{model_args.group_size}_residual{model_args.residual_length}_ratio{model_args.pruning_ratio}_")
            out_path = f"pred/{model_name}_{max_length}_{model_args.k_bits}bits_group{model_args.group_size}_residual{model_args.residual_length}_ratio{model_args.pruning_ratio}_/{dataset}.jsonl"

    ...
    def forward(
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            ...
            # ThinK
            key_mask = past_key_value[8]

            if key_states_quant_trans is not None:
                # ThinK
                mask_q = key_mask.unsqueeze(2)
                masked_query_states = query_states[mask_q].view(query_states.shape[0], query_states.shape[1], query_states.shape[2], -1)
                att_qkquant = cuda_bmm_fA_qB_outer(self.group_size, masked_query_states, key_states_quant_trans, 
            ...
            if key_states_full.shape[-2] == self.residual_length:
                ...
                # Think
                mask = key_mask.unsqueeze(2).expand(-1, -1, self.residual_length, -1)
                key_states_full = key_states_full[mask].reshape(key_states_full.shape[0], key_states_full.shape[1], self.residual_length, -1)
            ...
            if key_states_quant is not None:
                # Think
                key_states_quant, key_mask = key_pruner_query_driven(key_states_quant, query_states, self.pruning_ratio)
        ...
        # ThinK
        past_key_value = (key_states_quant_trans, key_states_full, key_scale_trans, key_mn_trans, value_states_quant, value_states_full, value_scale, value_mn, key_mask, kv_seq_len) if use_cache else None
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            ...
            if key_states_full.shape[-2] == self.residual_length:
                ...
                # Think
                mask = key_mask.unsqueeze(2).expand(-1, -1, self.residual_length, -1)
                key_states_full = key_states_full[mask].reshape(1, key_states_full.shape[1], self.residual_length, -1)
                key_states_quant_trans_new, key_scale_trans_new, key_mn_trans_new = triton_quantize_and_pack_along_last_dim(key_states_full.transpose(2, 3).contiguous(), 
                                                                                                                            self.group_size, 
                                                                                                                            self.k_bits)
                key_states_full = None
        ...
        past_key_value = (key_states_quant_trans, key_states_full, key_scale_trans, key_mn_trans, 
                          value_states_quant, value_states_full, value_scale, value_mn, key_mask, kv_seq_len) if use_cache else None
        attn_output = attn_output.reshape(bsz, q_len, self.hidden_size)
